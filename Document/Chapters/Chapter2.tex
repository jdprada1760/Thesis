...
On the other hand, scale transformations will be clearly given by a scalar multiplication. Consequently, a general transformation performed to the triangle relative coordinates can be represented by a scalar and a phase, or which is the same, a complex number. Taking this into account, we can codify the phase space information of the triangle from $\Psi$ in a Bloch sphere taking into account only its shape by omitting the rotations and scale transformations which leave it unchanged. To see that clearly, let us take the equivalence class representing a triangle shape:

\begin{equation}
\left[ \Psi \right] = \left\{ z\Psi | z \in \mathbb{C} \\ \left\{ 0 \right\} \right\}
\label{eq:equivalence}
\end{equation}

Now, to map the space of shapes $\left\{ \Psi / \sqrt{S} \right\} $ to a Bloch sphere, let us define the following quantity:

\begin{equation}
\vec{\zeta} = \frac{1}{S} \Psi^{\dagger}\vec{\sigma}\Psi
\end{equation}

The interpretation of this vector is 
